...
e_cashconstraint(hh,y,m)..   v_lives_expenses(hh,y,m) + v_crop_expenses(hh,y,m) +  sum(good, v_cons(hh,y,good))   =l=  v_cash(hh,y-1,'m12')$((ord(y) gt 1) and (ord(m) eq 1)) + v_cash(hh,y,m)$(ord(m) gt 1) + p_cash(hh)$((ord(y) eq 1) and (ord(m) eq 1))  ;
*v_hhold_expen(hh,y,m)   + v_cash(hh,y,m-1)$(ord(m) gt 1)
e_cashconstraint2(hh,y,m)..   v_lives_expenses(hh,y,m) + v_crop_expenses(hh,y,m) +  sum(good, v_cons(hh,y,good)) +   v_tot_credit_expenses(hh,y,m) =l=  v_cash(hh,y,m)$(ord(y) gt 1) + v_cash(hh,y,m-1)$(ord(m) gt 1)  + p_cash(hh)$((ord(y) eq 1) and (ord(m) eq 1)) + (1/12)*v_loan(hh)  ;
*+ p_offFarmIncome(hh)*(1/12)

e_cash_flow(hh,y,m)..          v_cash(hh,y,m) =e= p_cash(hh)$((ord(y) eq 1) and (ord(m) eq 1)) + v_cash(hh,y,m-1)$(ord(m) gt 1) + v_cash(hh,y-1,'m12')$((ord(m) eq 1) and (ord(y) gt 1))  + (v_income(hh,y,m-1)$(ord(m) gt 1)) - sum(good, v_cons(hh,y,good)*(1/12)) ;
...
e_cash_flow2(hh,y,m)..          v_cash(hh,y,m) =e= p_cash(hh)$((ord(y) eq 1) and (ord(m) eq 1)) + v_cash(hh,y,m-1)$(ord(m) gt 1) + v_cash(hh,y-1,'m12')$((ord(m) eq 1) and (ord(y) gt 1)) + (v_income(hh,y,m)$(ord(m) gt 1)) + (v_income(hh,y-1,'m12')$((ord(y) gt 1) and (ord(m) eq 1)))  - sum(good, v_cons(hh,y,good)) - (v_tot_credit_expenses(hh,y-1,'m12'))$((ord(m) eq 1) and (ord(y) gt 1))
...
*e_cons(hh,y,m,good)..                           v_cons(hh,y,m,good) =e= (0.75 + 0.95*(v_income(hh,y,m) - sum(good2,v_cons(hh,y,m,good2))))/1;

e_cons(hh,y,good)..                           v_cons(hh,y,good) =e= (1/12)*(0.75 + 0.95*(sum(m,v_income(hh,y,m)) - sum(good2,v_cons(hh,y,good2))))/1;
